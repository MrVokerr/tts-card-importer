...
### Daily token sync (primary: GitHub Actions)

Runs on GitHub-hosted runners daily at **06:00 UTC** (workflow `R2 token sync`). Your PC is not required.

Required repository secrets on `tts-card-importer` (Settings → Secrets → Actions):

| Secret | Purpose |
|--------|---------|
| `CLOUDFLARE_API_TOKEN` | Cloudflare API token with R2 object read/write (preferred) |
| `R2_ACCOUNT_ID` | Cloudflare account ID |
| `R2_BUCKET` | Bucket name (metadata bucket) |
| `R2_PUBLIC_BASE_URL` | Public HTTPS origin (e.g. `https://pub-….r2.dev`) |
| `R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY` | Optional S3-compatible alternative to the API token |

Do not commit API tokens or `.env` files. Manual retry: Actions → **R2 token sync** → Run workflow (`force=true` to ignore unchanged bulk).

Optional local/manual tools (`R2/scripts/sync-tokens-local.ps1`) exist for debugging only — not the daily path.
